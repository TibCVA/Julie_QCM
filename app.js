/* Julie la championne — Web App QCM (rev. 2025-08-13)
   - Deux datasets JSON: Annales 2020–2024 + Actualités 2024Q4
   - Mobile-first, PWA, révisions espacées, stats, mode examen
   - GitHub Pages friendly (chemins relatifs).
*/
...
(() => {
  ...
  const state = {
    ...
    stats: {
      ...
      q: {},
      // par jour (YYYY-MM-DD): nombre de validations
      days: {}
    },
  ...
  function safeAnswerIndex(raw){
    if (typeof raw === 'number'){
      if (raw>=0 && raw<=3) return raw;    // 0..3
      if (raw>=1 && raw<=4) return raw-1;  // 1..4
    }
    if (typeof raw === 'string'){
      const m = raw.trim().toUpperCase();
      if (['A','B','C','D'].includes(m)) return 'ABCD'.indexOf(m);
    }
    return 0;
  }

  // Retourne un tableau [{id,question,options[4],answerIndex,explanation,srcUrl,srcLabel}]
  function normalizeDataset(json, src){
    const out = [];
    const srcLabel = src.label;
    const srcUrl   = src.url;

    // 1) Obtenir une liste brute de questions, quel que soit le conteneur
    let list = [];
    if (Array.isArray(json)) list = json;
    else if (Array.isArray(json.questions)) list = json.questions;
      ...
      json.sections.forEach((sec,i)=>{
        if (Array.isArray(sec.questions)) list.push(...sec.questions);
      });
    }
    ...
    list.forEach((raw, i) => {
      // texte
      const qtext = (raw.question || raw.q || raw.text || '').toString().trim();
      if (!qtext){ return; }

      // options (accepte {choices:{A,B,C,D}} ou options[4])
      let options = [];
      if (raw.choices && typeof raw.choices === 'object'){
        const c = raw.choices;
        options = [c.A, c.B, c.C, c.D].map(x => (x ?? '').toString());
      } else if (Array.isArray(raw.options)) {
        options = raw.options.slice(0,4).map(x => (x ?? '').toString());
      }
      if (options.length !== 4 || options.some(o => !o.trim())){ return; }

      // réponse
      const answerIndex =
        safeAnswerIndex(raw.answer ?? raw.answer_index ?? raw.answerIndex);

      // explication
      const explanation =
        (raw.explanation || raw.answer_text || raw.solution || raw.comment || '').toString();

      // id stable
      const baseId = (raw.id ?? `${src.id}-${i}`).toString();
      const id = `${src.id}::${baseId}`;

      out.push({ id, question:qtext, options, answerIndex, explanation, srcUrl, srcLabel });
    });
  ...
  async function fetchJson(url){
    // Construit une URL absolue sûre (Safari iOS aime bien)
  ...
  function restore(){
    ...
    try{
      ...
      if (obj && obj._ver){
        ...
        state.selection = new Set(Array.isArray(obj.selection) ? obj.selection : SOURCES.map(s=>s.url));
      }
    }catch{ /* ignore */ }
  }
  ...
  function renderQuestion(){
    if (!state.pool.length){
      ...
      $('#q-choices').innerHTML=''; $('#q-feedback').textContent=''; $('#q-tag').textContent='—'; $('#q-index').textContent='—';
      return;
    }
  ...
  function validate(){
    ...
    fb.textContent = ok
      ? '✅ Bonne réponse !'
      : `❌ Mauvaise réponse. Solution : ${'ABCD'[q.answerIndex]}. ${q.explanation || ''}`.trim();

    // mise à jour SRS (une seule fois)
  ...
  function reveal(){
    ...
    $('#q-feedback').className='feedback';
    $('#q-feedback').textContent = `Réponse : ${'ABCD'[q.answerIndex]}. ${q.explanation || ''}`.trim();
  }
  ...
  // Expose quelques actions à l’extérieur si besoin (debug)
  window.JulieQCM = {
    version: APP_VER,
